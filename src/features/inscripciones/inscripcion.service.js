...
export const inscripcionService = {
  ...
  inscribirPaquete: async (data) => {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      const paramTiempo = await tx.parametros_sistema.findUnique({
      ...
      if (ultimoPago && ultimoPago.cuentas_por_cobrar.catalogo_conceptos) {
        ...
        if (ultimoPago.cuentas_por_cobrar.catalogo_conceptos.es_vigente === false) {
          esAlumnoLegacy = true;
          console.log(
            `👴 Alumno ${alumno_id} detectado como LEGACY. Se aplicarán precios antiguos.`
          );
        }
      }
      ...
      if (!conceptoAplicar) {
        ...
        throw new Error(
          `⛔ No existe un plan de precios ${tipoPrecio} configurado para un paquete de ${cantidadClases} días a la semana. Contacta administración.`
        );
      }
      ...
      if (esInscripcionAdicional) {
        ...
        if (!conceptoUnitario) {
          throw new Error(
            `⛔ ERROR DE CONFIGURACIÓN: No se encontró el concepto '${codigoUnitarioBuscar}'.`
          );
        }
      ...
      for (const idHorario of horario_ids) {
        ...
        if (esInscripcionAdicional && fechaCorte) {
          ...
          if (clasesRestantes > 0) {
            ...
            detalleCobro.push(
              `Upgrade ${esAlumnoLegacy ? '(Legacy)' : ''} ${horario.dia_semana} (${clasesRestantes} clases x S/ ${precioUnitarioOficial.toFixed(2)})`
            );
          } else {
          ...
          detalleCobro.push(
            `Mensualidad ${esAlumnoLegacy ? '(Legacy)' : ''} ${horario.dia_semana}`
          );
        }
      ...
      return {
        mensaje: esInscripcionAdicional
          ? 'Upgrade procesado correctamente.'
          : 'Inscripción creada. Realiza el pago.',
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      for (const candidato of candidatos) {
        ...
        if (!conceptoHeredado.activo) {
          console.log(
            `⛔ El plan ID ${conceptoHeredado.id} está desactivado por completo. No se renueva.`
          );
          continue;
        }
        ...
        if (conceptoHeredado.cantidad_clases_semanal === totalCursosActivos) {
          ...
          console.log(
            `⚠️ Alumno ${alumnoId} cambió frecuencia (Tiene ${totalCursosActivos}, Plan era ${conceptoHeredado.cantidad_clases_semanal}).`
          );
        }
      }
