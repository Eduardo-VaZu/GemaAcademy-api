...
const limpiarReservasZombies = async () => {
  ...
  const zombies = await prisma.inscripciones.findMany({
    where: {
      ...
      // 🔥 CAMBIO 1: Buscamos por la fecha de creación inmutable
      ...
      await prisma.$transaction(async (tx) => {
        ...
        const deuda = await tx.cuentas_por_cobrar.findFirst({
          where: {
            ...
            estado: 'PENDIENTE',
            creado_en: {
              // 🔥 CAMBIO 2 y 3: Amarramos la búsqueda a la creación del zombie
              gte: new Date(zombie.creado_en.getTime() - 30000),
              lte: new Date(zombie.creado_en.getTime() + 30000),
            },
          },
