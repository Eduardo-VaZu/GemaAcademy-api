...
export const claseSchema = {
  reprogramarMasivoSchema: z.object({
    ...
    fecha_origen: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Fecha origen debe tener formato YYYY-MM-DD'),
    ...
    fecha_destino: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Fecha destino debe tener formato YYYY-MM-DD'),
    motivo: z.string().min(3, 'El motivo debe tener al menos 3 caracteres').max(200, 'El motivo es muy largo'),
  }),
};
