  ...
  registrarPago: async (data) => {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      const deuda = await tx.cuentas_por_cobrar.findUnique({
        where: { id: Number.parseInt(deuda_id) },
      });
      ...
      if (!isNaN(metodo_pago)) {
        metodoPagoId = Number.parseInt(metodo_pago);
      } else {
        ...
        if (!metodoEncontrado) {
          ...
          if (!defaultMetodo)
            throw new Error('Error técnico: No hay métodos de pago configurados.');
  ...
  validarPago: async (data) => {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      let pago = await tx.pagos.findUnique({
        where: { id: Number.parseInt(pago_id) },
      ...
      if (accion === 'APROBAR' && monto_real_confirmado) {
        ...
        if (montoAdmin !== Number(pago.monto_pagado)) {
          console.log(
            `👮‍♂️ Corrección de Admin: Usuario dijo ${pago.monto_pagado}, Real es ${montoAdmin}`
          );
      ...
      if (!esAprobado) {
        // Caso 1: Rechazado ❌
        mensajeFinal =
          '⛔ PAGO RECHAZADO. La inscripción ha vuelto a estado PENDIENTE (o será eliminada por el sistema).';
